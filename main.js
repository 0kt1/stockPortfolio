...
$(document).ready(function() {
  ...
  $("#search").autocomplete({
    source: function(request, response) {
      // Make an AJAX request to fetch matching items
      $.ajax({
        type: "GET",
        url: `https://api.iextrading.com/1.0/stock/${search}/symbols`,
        dataType: "json",
        success: function(data) {
          ...
          // Map the filtered data to an array of labels
          const labels = filteredData.map(item => item.symbol);

          // Pass the labels to the response callback
          response(labels);
        }
      });
    },
    ...
    select: function(event, ui) {
      // Handle selection from the autocomplete dropdown here
